...
const Home = () => {
    ...
    const [answers, setAnswers] = useState(new Array<Answer>());
    ...
    useEffect(() => {
        const questions = ref(database, '/');
        onValue(questions, (snapshot) => {
            const data = snapshot.val();
            setCurrentQuestion(data.questions[data.currentQuestion]);
            setAnswers(data.currentQuestion >= 0 ? data.questions[data.currentQuestion].answers : []);
            let totalPoints = 0;
    ...
    return (
        <Container maxWidth="lg">
            <Box sx={{ flexGrow: 2 }}>
                ...
                <div>
                    <h1>{currentQuestion.revealed ? currentQuestion.text : "Coming..."}</h1>
                </div>
                <Grid container spacing={2}>
                    <Grid item xs={6}>
