...
const FlashRoundService = {
    get: async () => {
        const querySnapshot = await getDocs(collection(firestore, FLASH_ROUND_COLLECTION));
    ...
    getById: async (id: string) => {
        const documentSnapshot = await getDoc(doc(firestore, FLASH_ROUND_COLLECTION, id));

        return documentSnapshot.data();
    },
    insert: async (document: FlashRound) => {
        const docRef = await addDoc(collection(firestore, FLASH_ROUND_COLLECTION), document);
        return docRef;
    },
